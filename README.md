...
```sh
git clone https://github.com/nagarajRPoojari/orange
cd orange
```
</details>
...
This section will guide you to setup a fully-functional local copy of `orange`
...
from [`orange's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `orange`, run the
command
...
The pre-commit hooks used by `orange` are located within the
...
Tests in `orange` are classified as *fast* and *slow* - depending
...
|      `run`     	| Run *orange*                           	|       NA      	|
|  `docker-gen`  	| Create production docker image for *orange* 	|     docker    	|
|  `docker-debug`  	| Create debug-friendly docker image for *orange* 	|     docker    	|
...
```text
orange_<version>_<target-os>_<architecture>.<extension>
```
...
```sh
orange_v1.2.1_windows_x86_64.exe
```
...
To run `orange` in a docker container, read the instructions in
[docker section](./docker).

### Running `orange`

To run orange, use the command
