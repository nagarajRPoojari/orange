...
a docker containers for *orange*!
...
```bash
make docker-gen IMAGE=orange VERSION=latest
```
...
```bash
make clean-docker IMAGE=orange VERSION=latest
```


## Debug Images

The [dockerfile](./Dockerfile) for *orange* uses
...
```bash
make docker-debug IMAGE=orange-debug VERSION=latest
```
...
```bash
docker run orange-debug
```

You can run a debug image, and directly `shell` into it with

```bash
docker run --rm -it orange-debug bash
```
...
```bash
make clean-docker IMAGE=orange-debug VERSION=latest
```
